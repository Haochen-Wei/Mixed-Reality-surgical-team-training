...
def main(c):
    ...
    while key != 113:  # for 'q' key
        ...
        retval, frame = cap.read()
        # Extract left and right images from side-by-side
        g_lr=cv2.cvtColor(frame,cv2.COLOR_RGB2GRAY)
        [_,g_lr]=cv2.threshold(g_lr,15,255,cv2.THRESH_BINARY)
        left_right_image = np.split(g_lr, 2, axis=1)


        # left_right_image = np.split(frame, 2, axis=1)
        g_l = cv2.remap(left_right_image[0], map_left_x, map_left_y, interpolation=cv2.INTER_LINEAR)
        g_r = cv2.remap(left_right_image[1], map_right_x, map_right_y, interpolation=cv2.INTER_LINEAR)
        #Binary method
        # g_l=cv2.cvtColor(left_rect,cv2.COLOR_RGB2GRAY)
        # g_r=cv2.cvtColor(right_rect,cv2.COLOR_RGB2GRAY)
        # max_l=np.amax(g_l)
    ...
    #================================================================================================================
    #This block is used to show the left and right filtered points, please only use when debug, otherwise it will drop significently of performance  
        # Show the line point
        # for i in range(len(left_point)):
        #     for j in range(len(left_point[i])):
        #         cv2.circle(left_rect,(left_point[i][j][0],left_point[i][j][1]),1,(0,255,0))
